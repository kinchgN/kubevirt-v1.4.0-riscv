...
/*
 * riscv64 utilities are in the webhooks package because they are used both
...
var _falseRV bool = false

const (
	defaultCPUModelRISCV64 = v1.CPUModeHostPassthrough
)

// ValidateVirtualMachineInstanceRISCV64Setting is a validation function for validating-webhook to filter unsupported setting on RISC-V
func ValidateVirtualMachineInstanceRISCV64Setting(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec) []metav1.StatusCause {
	...
	validateBootOptionsRISCV64(field, spec, &statusCauses)
	validateCPUModelRISCV64(field, spec, &statusCauses)
	validateDiskBusRISCV64(field, spec, &statusCauses)
	validateWatchdogRISCV64(field, spec, &statusCauses)
	validateSoundDeviceRISCV64(field, spec, &statusCauses)
	return statusCauses
}

func validateBootOptionsRISCV64(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec, statusCauses *[]metav1.StatusCause) {
	if spec.Domain.Firmware != nil && spec.Domain.Firmware.Bootloader != nil {
		if spec.Domain.Firmware.Bootloader.BIOS != nil {
			*statusCauses = append(*statusCauses, metav1.StatusCause{
				...
				Message: "RISC-V does not support bios boot, please change to uefi boot",
		...
		if spec.Domain.Firmware.Bootloader.EFI != nil {
			...
			if spec.Domain.Firmware.Bootloader.EFI.SecureBoot == nil || (spec.Domain.Firmware.Bootloader.EFI.SecureBoot != nil && *spec.Domain.Firmware.Bootloader.EFI.SecureBoot) {
				*statusCauses = append(*statusCauses, metav1.StatusCause{
					Type:    metav1.CauseTypeFieldValueNotSupported,
					Message: "UEFI secure boot is currently not supported on RISC-V Arch",
...
func validateCPUModelRISCV64(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec, statusCauses *[]metav1.StatusCause) {
	if spec.Domain.CPU != nil && (&spec.Domain.CPU.Model != nil) && spec.Domain.CPU.Model == "host-model" {
		*statusCauses = append(*statusCauses, metav1.StatusCause{
			Type:    metav1.CauseTypeFieldValueNotSupported,
			Message: "RISC-V does not support CPU host-model",
			Field:   field.Child("domain", "cpu", "model").String(),
		})
	}
}

func validateDiskBusRISCV64(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec, statusCauses *[]metav1.StatusCause) {
	if spec.Domain.Devices.Disks != nil {
		...
		for i, disk := range spec.Domain.Devices.Disks {
			if disk.Disk != nil && !checkIfBusAvailable(disk.Disk.Bus) {
				*statusCauses = append(*statusCauses, metav1.StatusCause{
					Type:    metav1.CauseTypeFieldValueNotSupported,
					Message: "RISC-V does not support this disk bus type, please use virtio or scsi",
...
func validateWatchdogRISCV64(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec, statusCauses *[]metav1.StatusCause) {
	if spec.Domain.Devices.Watchdog != nil {
		*statusCauses = append(*statusCauses, metav1.StatusCause{
			Type:    metav1.CauseTypeFieldValueNotSupported,
			Message: "RISC-V does not support Watchdog device",
			Field:   field.Child("domain", "devices", "watchdog").String(),
		})
	}
}

func validateSoundDeviceRISCV64(field *k8sfield.Path, spec *v1.VirtualMachineInstanceSpec, statusCauses *[]metav1.StatusCause) {
	if spec.Domain.Devices.Sound != nil {
		*statusCauses = append(*statusCauses, metav1.StatusCause{
			Type:    metav1.CauseTypeFieldValueNotSupported,
			Message: "RISC-V does not support sound device",
